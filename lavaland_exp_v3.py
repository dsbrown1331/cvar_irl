...
#I'm going to see if just letting the sampling run wild will make our method look better. Seems like it is more robust.
...
experiment_name = 'grid5x5_ploss_lavaocc_v3.txt'
import os
...
for t in range(num_trials):
    ...
    #train mdp
    mdp_env_A = mdp_worlds.lavaland_small_somegrass(contains_lava=False)
    #test mdp ((probably) has lava)
    mdp_env_B = mdp_worlds.lavaland_small_somegrass(contains_lava=True)
    ...
    mean_lava = np.dot(map_u, stacked_weights)[3]
